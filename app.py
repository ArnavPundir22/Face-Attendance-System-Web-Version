...
The application is split into focused modules:
  ...
  utils/auth_helpers.py       — rate-limiting, OTP, admin checks
  utils/face.py               — InsightFace model + encoding I/O (numpy, no pickle)
  utils/mail.py               — email delivery helpers
  blueprints/auth.py          — /login, /logout, /register, /forgot_password
...
def require_login():
    ...
    public_paths = {'/login', '/forgot_password', '/favicon.ico'}
