...
The **Face Attendance System** is a modern web application that completely eliminates manual roll-calls using state-of-the-art facial recognition. A teacher uploads a classroom photo (or captures one with a webcam), and the system instantly identifies every student in the image, marking them present in a secure PostgreSQL database.
...
*   **[🏗️ Architecture Guide](docs/ARCHITECTURE.md)**: Deep dive into the stateless system flow, the AI inference logic, and view the **Mermaid flowcharts**.
...
*   **[👨‍💼 Administrator Guide](docs/ADMIN_GUIDE.md)**: A user manual covering how to grant admin privileges, register students, and accurately capture group attendance.
...
| 🚀 **Stateless Architecture** | Embeddings are stored natively in Supabase using `pgvector`. No local `.npy` files! |
...
| 🛡️ **Re-attendance Prevention** | Blocks duplicate marks within a configurable time window (e.g., 60 minutes). |
| 📊 **Real-time Dashboard** | Filterable attendance table with an instant **CSV Download** feature for reporting. |
| 👨‍💼 **Admin Management** | Register new students, update records, and view a 7-day attendance trend chart. |
| 🎨 **Premium UI/UX** | Dark neon glassmorphism aesthetic built with TailwindCSS and Lucide Icons. |
| 🔐 **Secure Auth** | Passwords hashed with `bcrypt`, combined with strict session management. |
